# Moist Canvas

Moist Canvas 是一个本地运行的 AI 无限画布。它通过在线 API 服务生成和编辑图片 / 视频，不需要本地 GPU，也不需要安装 ComfyUI。

## 给普通用户的使用方法（Windows 64 位）

1. 解压整个项目文件夹。
2. 双击 `安装依赖.bat`。
3. 等待脚本自动下载便携 Python，并安装项目依赖。
4. 安装完成后，双击 `运行文件.bat`。
5. 浏览器会自动打开 `http://127.0.0.1:3000/`。
6. 第一次使用时，在网页里的 API 设置中填写你的 API Key。

> 注意：第一次安装需要联网。用户不需要自己安装 Python。

## 运行时需要什么

- Windows 64 位系统。
- 第一次安装时需要联网，用来下载便携 Python 和 Python 依赖库。
- 需要浏览器，Windows 自带 Edge 即可。
- 生成图片 / 视频时需要可用的 API Key，例如 APIMart、ModelScope 或 OpenAI 兼容服务。

项目不需要：

- 本地 GPU。
- ComfyUI。
- Node.js / npm。
- 数据库。

## 开发者运行方式

如果你已经安装了 Python 3.10+，也可以直接在项目目录执行：

```powershell
python -m pip install -r requirements.txt
python main.py
```

然后打开：

```text
http://127.0.0.1:3000/
```

## 数据目录

- `data/`：画布和配置数据。
- `data/canvases_v2/`：画布存档。
- `output/`：生成的图片 / 视频。
- `history.json`：生成历史索引。
- `API/.env`：本地 API Key / 环境变量。
- `runtime/`：`安装依赖.bat` 自动下载的便携 Python 和依赖。

不要把 `API/.env`、`output/`、`history.json`、`runtime/` 发给别人，里面可能包含你的私人配置、本机缓存或生成内容。

## 生成测试 ZIP 包

在项目目录运行：

```powershell
powershell -ExecutionPolicy Bypass -File scripts\build_test_zip.ps1
```

生成的文件在：

```text
dist\MoistCanvas-Test.zip
```

这个 ZIP 是给别人试用的干净包：解压后先运行 `安装依赖.bat`，再运行 `运行文件.bat`。
